...
def num2barLight(number, leds):
    GPIO.output(leds, [0] * (8 - number // 30) + [1] * (number // 30))


if __name__ == "__main__":
    GPIO.setmode(GPIO.BCM)
    leds = [26, 19, 13, 6, 5, 11, 9, 10]
    leds_bar = [21, 20, 16, 12, 7, 8, 25, 24]
    config = [0 for _ in range(8)]
    GPIO.setup(leds, GPIO.OUT)
    GPIO.setup(leds_bar, GPIO.OUT)
    GPIO.output(leds + leds_bar, config + config)
